...
cat <<EOF | sed -e s/CODENAME/`lsb_release -cs`/g >> /tmp/sources.list
...
apt-get -qq update
apt-get install -y git cmake gcc-aarch64-linux-gnu libc6-arm64-cross libc6:arm64 libmpfr-dev:arm64 libgomp1:arm64 libmpfr-dev binfmt-support qemu qemu-user-static libfftw3-dev:arm64 libssl-dev libssl-dev:arm64
