...
double sinpifr(double d) {
  ...
  mpfr_sin(frd, frd, GMP_RNDN);
  double ret = mpfr_get_d(frd, GMP_RNDN);

  mpfr_clears(frpi, frd, NULL);
  return ret;
}

double cospifr(double d) {
  mpfr_t frpi, frd;
  mpfr_inits(frpi, frd, NULL);
...
Sleef_double2 child_sincos(double x) {
  char str[256];
  uint64_t u, v;
...
Sleef_double2 child_sincospi_u05(double x) {
  char str[256];
  uint64_t u, v;

  sprintf(str, "sincospi_u05 %" PRIx64 "\n", d2u(x));
  write(ptoc[1], str, strlen(str));
  if (readln(ctop[0], str, 255) < 1) stop("child_sincospi_u05");
  sscanf(str, "%" PRIx64 " %" PRIx64, &u, &v);
...
Sleef_double2 child_sincospi_u35(double x) {
  char str[256];
  uint64_t u, v;

  sprintf(str, "sincospi_u35 %" PRIx64 "\n", d2u(x));
  write(ptoc[1], str, strlen(str));
  if (readln(ctop[0], str, 255) < 1) stop("child_sincospi_u35");
  sscanf(str, "%" PRIx64 " %" PRIx64, &u, &v);
...
void do_test() {
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(double) && success;i++) {
      Sleef_double2 q = child_sincos(xa[i]);
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(double) && success;i++) {
      Sleef_double2 q = child_sincospi_u05(xa[i]);
      if (!cmpDenorm(q.x, sinpifr(xa[i]))) {
	fprintf(stderr, "arg = %.20g, test = %.20g, correct = %.20g\n", xa[i], q.x, sinfr(xa[i]));
	success = false;
	break;
      }
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(double) && success;i++) {
      Sleef_double2 q = child_sincospi_u35(xa[i]);
      if (!cmpDenorm(q.x, sinpifr(xa[i]))) {
	fprintf(stderr, "arg = %.20g, test = %.20g, correct = %.20g\n", xa[i], q.x, sinfr(xa[i]));
	success = false;
	break;
...

  {
	...
	fprintf(stderr, "arg = %.20g, test = %.20g, correct = %.20g\n", xa[i], child_cos(xa[i]), cosfr(xa[i]));
	success = false;
	break;
      }
    }
...

  {
	...
	fprintf(stderr, "arg = %.20g, test = %.20g, correct = %.20g\n", xa[i], child_tan(xa[i]), tanfr(xa[i]));
	success = false;
	break;
      }
    }
...

  {
    ...
    double xa[] = { NAN, POSITIVE_INFINITY, NEGATIVE_INFINITY, 10000, -10000, +0.0, -0.0, -1e+10, +1e+10 };
...

  {
    ...
    double xa[] = { NAN, POSITIVE_INFINITY, NEGATIVE_INFINITY, 0, -1, nextafter(0, -1), +0.0, -0.0 };
...

  {
    ...
    double xa[] = { NAN, POSITIVE_INFINITY, NEGATIVE_INFINITY, 0, -1, nextafter(-1, -2), -2, +0.0, -0.0 };
...

  {
    ...
    {
      double t = child_ilogb(NAN);
      if (t != INT_MAX && t != INT_MIN) success = false;
    }

    {
      double t = child_ilogb(0);
      if (t != INT_MIN && t != -INT_MAX) success = false;
    }
  ...
  
  {
    ...
    for(d = -10;d < 10;d += 0.0002) {
      ...
      max = fmax(max, u);
      if (u > 20) {
	fprintf(stderr, "arg = %.20g, correct = %.20g, test = %.20g, ulp = %g\n", d, (double)c, q, u);
      }
    }
    ...
    for(i=(int64_t)-1e+14;i<(int64_t)1e+14;i+=(int64_t)1e+11) {
...

  {
    ...
    for(d = -10;d < 10;d += 0.0002) {
      Sleef_double2 q = child_sincos(d);
      long double c = sinlfr(d);
      double u = fabsl((q.x - c) / ulp(c));
      max = fmax(max, u);
      if (u > 1000) {
