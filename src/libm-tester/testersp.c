...
double flushToZero(double y) {
  if (enableFlushToZero && fabs(y) < 1.17549e-38) y = copysign(0.0, y);
  return y;
}
...
Sleef_float2 child_sincosf(float x) {
  char str[256];
  uint32_t u, v;
...
Sleef_float2 child_sincospif_u05(float x) {
  char str[256];
  uint32_t u, v;

  sprintf(str, "sincospif_u05 %x\n", f2u(x));
  write(ptoc[1], str, strlen(str));
  if (readln(ctop[0], str, 255) < 1) stop("child_sincospif_u05");
  sscanf(str, "%x %x", &u, &v);
...
Sleef_float2 child_sincospif_u35(float x) {
  char str[256];
  uint32_t u, v;

  sprintf(str, "sincospif_u35 %x\n", f2u(x));
  write(ptoc[1], str, strlen(str));
  if (readln(ctop[0], str, 255) < 1) stop("child_sincospif_u35");
  sscanf(str, "%x %x" PRIx64, &u, &v);
...
void do_test() {
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(float) && success;i++) {
      Sleef_float2 q = child_sincosf(xa[i]);
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(float) && success;i++) {
      Sleef_float2 q = child_sincospif_u05(xa[i]);
...

  {
    ...
    boolean success = true;
    for(i=0;i<sizeof(xa)/sizeof(float) && success;i++) {
      Sleef_float2 q = child_sincospif_u35(xa[i]);
      if (!cmpDenorm(q.x, sinpifr(xa[i]))) {
	fprintf(stderr, "arg = %.20g, test = %.20g, correct = %.20g\n", xa[i], q.x, sinfr(xa[i]));
	success = false;
	break;
  ...
  {
    fprintf(stderr, "cos in sincospif_u05 denormal/nonnumber test ... ");
  ...
  {
    fprintf(stderr, "cos in sincospif_u35 denormal/nonnumber test ... ");
...

  {
    ...
    float xa[] = { POSITIVE_INFINITYf, NEGATIVE_INFINITYf, -1, -2, };
...

  {
    ...
    for(d = -10;d < 10;d += 0.0002) {
      Sleef_float2 q = child_sincosf(d);
